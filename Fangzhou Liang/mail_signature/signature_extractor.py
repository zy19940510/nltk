...
for each_file in file_list:
    f = open(each_file, 'r')
    lines = f.readlines()
    name_tuple = name_reg.findall(lines[0].encode('utf-8').decode('utf-8'))[0]
    ...
    for each_line in lines[1:]:
        # parse phone number
        phone = phone_reg.findall(each_line.encode('utf-8').decode('utf-8'))
        if phone != []:
        ...
        if jieba_noun != [] or nltk_noun != []:
            org.append(each_line.strip().encode('utf-8').decode('utf-8'))
    file.write('File Name:' + each_file+'\n')
    file.write('Name:' + ''.join(name)+'\n')
    file.write('Phone Num:' + ''.join(phone_num)+'\n')
    file.write('Email:' + ''.join(email)+'\n')
    file.write('Org:' + ''.join(org)+'\n')
    file.write('-' * 15+'\n')
file.close()
